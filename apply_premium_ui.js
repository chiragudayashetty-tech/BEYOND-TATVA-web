...
// The block we are looking for is essentially:
// <div class="old eo">Regular price ₹2,499</div>
// <div class="old ko kn-font">ಸಾಮಾನ್ಯ ಬೆಲೆ ₹2,499</div>
// <div><span class="bigprice">₹799</span><span class="savepill">SAVE ₹1,700</span></div>

const oldPriceHTMLRegex = /<div class="old eo">Regular price ₹2,499<\/div>[\s\S]*?<div><span class="bigprice">₹799<\/span><span class="savepill">SAVE ₹1,700<\/span><\/div>/;

const newPremiumBlock = \`<style>
/* ── SUPER PREMIUM ANIMATED PRICE BOX ── */
.premium-price-box {
...
.premium-price-box.loaded { transform: scale(1); opacity: 1; }
.premium-price-box::before {
    content: ''; position: absolute;
    top: 0; left: -100%; width: 50%; height: 100%;
    background: linear-gradient(to right, transparent, rgba(255,255,255,0.06), transparent);
    transform: skewX(-20deg); animation: shineSweep 4s infinite 2s;
}
.premium-price-box.glowing {
    ...
    border-top: 1px solid rgba(250, 204, 21, 0.4); border-bottom: 1px solid rgba(250, 204, 21, 0.1);
    transform: translateY(-4px);
}
.ppb-top {
    font-size: 0.95rem; color: var(--muted); margin-bottom: 8px; text-transform: uppercase;
    letter-spacing: 2px; font-weight: 600; opacity: 0; transform: translateY(-10px);
    transition: all 0.8s cubic-bezier(0.34, 1.56, 0.64, 1);
}
.ppb-top.show-strike { opacity: 1; transform: translateY(0); }
.ppb-strike { text-decoration: line-through; color: var(--red); opacity: 0.85; font-weight: 500; margin-left: 8px; }
.ppb-main {
    display: flex; align-items: center; justify-content: center; gap: 6px; transform: scale(0.9);
    opacity: 0; transition: all 0.6s cubic-bezier(0.34, 1.56, 0.64, 1) 0.2s;
}
.premium-price-box.loaded .ppb-main { transform: scale(1); opacity: 1; }
.ppb-currency { font-size: 2.2rem; font-weight: 700; color: #f1f5f9; margin-top: -16px; text-shadow: 0 2px 10px rgba(0,0,0,0.4); }
.ppb-num {
    font-family: var(--h); font-size: 5rem; font-weight: 800; letter-spacing: -2px; line-height: 1;
    color: #fff; text-shadow: 0 4px 16px rgba(0,0,0,0.4); font-variant-numeric: tabular-nums;
    will-change: transform, color; transition: color 0.4s ease, transform 0.2s ease;
}
.ppb-num.counting { color: #cbd5e1; transform: scale(1.08); text-shadow: 0 0 25px rgba(255,255,255,0.2); }
.ppb-num.done {
    background: linear-gradient(180deg, #FDE68A 0%, #F59E0B 50%, #D97706 100%);
    -webkit-background-clip: text; -webkit-text-fill-color: transparent;
    text-shadow: none; filter: drop-shadow(0 4px 12px rgba(245, 158, 11, 0.4)); transform: scale(1);
}
.ppb-save {
    position: absolute; bottom: -16px; left: 50%; transform: translateX(-50%) translateY(20px) scale(0.8);
...
.ppb-save.show {
    opacity: 1; transform: translateX(-50%) translateY(0) scale(1); animation: pulseSave 2.5s infinite 1s;
}
...
if (oldPriceHTMLRegex.test(html)) {
    html = html.replace(oldPriceHTMLRegex, newPremiumBlock);
    console.log("Successfully replaced the hero price block!");
    fs.writeFileSync('index - Copy.html', html, 'utf8');
} else {
    console.log("Error: Could not find the old price HTML using regex.");
}
