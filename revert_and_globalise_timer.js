...
// 1. Revert HTML of the premium price box back to the Gold Rolling animation structure
const redHTML = /<div class="premium-price-box" id="ppbHero">[\s\S]*?<\/div>\s*<\/div>\s*<\/div>/;

const goldRollingHTML = `
...
html = html.replace(redHTML, goldRollingHTML.trim());

// 2. Revert the CSS back to the ultra-polished Gold Rolling animation
const redCSS = /\/\*\s*═+\s*PREMIUM RED COUNTDOWN UI[\s\S]*?\}\s*\}/;

const ultraPremiumCSS = `
        /* ═══ PREMIUM COUNTDOWN PRICE (ULTRA POLISHED) ═══ */
        .premium-price-box {
            text-align: center;
            margin: 15px 0 25px;
            padding: 22px 18px;
...
html = html.replace(redCSS, ultraPremiumCSS.trim());


// 3. Remove the localized red timer javascript and add the Gold Rolling logic back
...
document.addEventListener('DOMContentLoaded', () => {
    // 1. Roll Animation for Hero Price Box
            ...
            if(entries[0].isIntersecting && !animatedp) {
                ...
                function countEngine(currentTime) {
                    ...
                    if (progress < 1) requestAnimationFrame(countEngine);
                        ...
                        setTimeout(() => pNum.style.transform = 'scale(1)', 500);
                    }
                }
...
html = html.replace(badTimerJSRegex, goldRollingJS.trim());

// 4. Modying the Global Site Timers (Sticky Bar, Countdown sections) to EXACTLY 4h 30m 52s
// The previous timer logic uses a hardcoded deadline Date object or logic inside the global scripts we restored.
// We will replace `const DEADLINE = new Date(); DEADLINE.setDate(...)` with a precise countdown logic.

html = html.replace(/const DEADLINE = new Date\(\);[^;]+;(?:\s*DEADLINE\.[^;]+;)*\s*DEADLINE\.setHours\(23, 59, 59, 999\);/g,
    \`// Force exactly 4h 30m 52s from load
          let totalSecondsRemaining = (4 * 3600) + (30 * 60) + 52;
          const DEADLINE = new Date(new Date().getTime() + totalSecondsRemaining * 1000);
\`);

fs.writeFileSync('index - Copy.html', html, 'utf8');
console.log('✅ Reverted Hero to Gold Rolling Animation');
console.log('✅ Injected global site timer to 4 hours 30 mins 52 secs limit');
